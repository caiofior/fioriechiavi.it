...
if (array_key_exists('sEcho', $_REQUEST)) {
      ...
      foreach($taxaObservationColl->getItems() as $key => $taxaObservation) {
         ...
         foreach($columns as $column) {
            ...
            $cid = urlencode(mcrypt_encrypt($GLOBALS['db']->config->crypt->cipher,$GLOBALS['db']->config->crypt->key,$taxaObservation->getData('id'),$GLOBALS['db']->config->crypt->mode,substr($GLOBALS['db']->config->crypt->iv,0,$vector_size)));
            if ($column == 'actions') {
