...
if (array_key_exists('sEcho', $_REQUEST)) {
      ...
      foreach($taxaObservationColl->getItems() as $key => $taxaObservation) {
         ...
         foreach($columns as $column) {
            ...
            $cid = urlencode(mcrypt_encrypt($GLOBALS['db']->config->crypt->cipher,$GLOBALS['db']->config->crypt->key,$GLOBALS['db']->config->crypt->seed.$taxaObservation->getData('id'),$GLOBALS['db']->config->crypt->mode,substr($GLOBALS['db']->config->crypt->iv,0,$vector_size)));
            if ($column == 'actions') {
   ...
   if (
      ...
      if (array_key_exists('submit', $_REQUEST) && $this->formIsValid()) {
         ...
         if (array_key_exists('cid', $_REQUEST)) {
            $vector_size = mcrypt_get_iv_size($GLOBALS['db']->config->crypt->cipher,$GLOBALS['db']->config->crypt->mode);
            $_REQUEST['id'] = preg_replace('/^'.$GLOBALS['db']->config->crypt->seed.'/','',mcrypt_decrypt($GLOBALS['db']->config->crypt->cipher,$GLOBALS['db']->config->crypt->key,  urldecode($_REQUEST['cid']),$GLOBALS['db']->config->crypt->mode,substr($GLOBALS['db']->config->crypt->iv,0,$vector_size)));
         }
