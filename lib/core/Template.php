...
class Template {
   ...
   public static function encodeId ($id) {
      if (extension_loaded('openssl')) {
         $ivlen = openssl_cipher_iv_length($GLOBALS['db']->config->crypt->cipher);
         if (strlen($GLOBALS['db']->config->crypt->iv) < $ivlen) {
             throw new Exception('The mcrypt iv is too short, it must be at least '.$ivlen.' long',1509141707);
         }
